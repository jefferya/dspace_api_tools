# The Class to map Jupiter Item to DSpace
# based on : https://gist.github.com/lagoan/839cf8ce997fa17b529d84776b91cdac
#   without the last line that instantiates the class and runs

class CollectionCSVExporter
  ...
  def handle_manual_value(item, key)
    ...
    when 'manual_edit_history'
      ...
      edit_history = {
        jupiter_edit_history: clean_up_edit_history(item.versions)
      }.to_json.to_s
  ...
  def run
    ...
    Collection.find_each do |collection|
      CSV.open("#{directory_path}/#{collection.id}.csv", 'wb', write_headers: true, headers: item_headers) do |csv|
